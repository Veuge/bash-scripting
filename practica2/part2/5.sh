...
# Si no tiene ninguna sesion abierta mostrar su directorio HOME.
...
if [ $IS -eq 0 ]; then
    ...
    if [ $NSESSIONS -eq 0 ]; then
	printf "El usuario ${USER} no está conectado actualmente. Su directorio HOME es:\n";
	echo ~$USER;
    elif [ $NSESSIONS -eq 1 ]; then
	printf "El usuario ${USER} tiene una sesion abierta. Los procesos iniciados por este usuario son:\n";
	echo -e "$(ps r -u $USER)";
    else
